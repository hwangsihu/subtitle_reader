...
class Menu(wx.Menu):
	def __init__(self, main):
		...
		self.onlyReadManagersChat = self.youtube.AppendCheckItem(
			wx.ID_ANY,
			_("Read only messages from the moderator (&M)"),
		)
		...
		self.omitChatGraphic = self.youtube.AppendCheckItem(
			wx.ID_ANY,
			_("Skip the picture name when reading the chat (&G)"),
		)
		...
		self.crunchyrollSetupMenuItem = self.AppendSubMenu(
			self.crunchyrollSetup,
			_("Crunchyroll - Setup (&K)"),
		)
		...
		self.contactDeveloperMenuItem = self.AppendSubMenu(
			self.contactDeveloper,
			_("contact the developer (&C)"),
		)
		...
		self.contactUseLine = self.contactDeveloper.Append(
			wx.ID_ANY,
			"Line, id:Maxe0310 " + _("Click to copy"),
		)
		self.contactUseDiscord = self.contactDeveloper.Append(
			wx.ID_ANY,
			"Discord, ID:maxe0310 " + _("Click to copy"),
		)
		...
		for platform in SubtitleExtractor.extractors:
			self.platforms.Append(
				wx.ID_ANY,
				platform.info["name"] + _(" State:") + platform.info["status"],
				platform.info["url"],
			)
...
class UpdateDialog(wx.Dialog):
	def __init__(self, version):
		super(UpdateDialog, self).__init__(
			gui.mainFrame,
			wx.ID_ANY,
			title=_("Subtitle reader V") + str(version) + _(" Version information"),
		)
		...
		self.changelogText = wx.TextCtrl(
			self,
