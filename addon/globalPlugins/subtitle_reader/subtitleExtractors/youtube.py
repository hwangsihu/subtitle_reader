...
class Youtube(SubtitleExtractor):
	...
	def getSubtitleContainer(self):
		...
		# 沉浸式翻譯
		container = container or find(
			videoPlayer.firstChild,
			"next",
			"id",
	...
	def getChatContainer(self):
		...
		self.chatContainerSearchObject = search(
			obj,
			lambda obj: (
				"yt-live-chat-item-list-renderer" in obj.IA2Attributes.get("class")
				and obj.IA2Attributes.get("id") == "items"
			)
			or obj.IA2Attributes.get("id") == "chat",
			self.onFoundChatContainer,
	...
	def readChat(self):
		...
		self.chatObject = (
			self.chatContainer.lastChild
			if not self.chatObject or not self.chatObject.role
			else self.chatObject.next
		)
		...
		self.chatSearchObject = search(
			self.chatObject.firstChild,
			self.chatCondition,
			self.onFoundChatObject,
			continueOnFound=True,
		)

	def chatCondition(self, obj):
		if "ytd-sponsorships-live-chat-gift-redemption-announcement-renderer" in obj.IA2Attributes.get(
			"class",
		):
			return
	...
	def onFoundChatObject(self, chat):
		...
		sender = _("{name}.\r\n{isVerified}.\r\n{isAdmin}.\r\n").format(
			name=self.chatSender,
			isVerified=self.chatSenderIsVerified,
			isAdmin=self.chatSenderIsAdmin,
		)
		if (
			conf["readChatSender"]
			or self.chatSenderIsOwner
			or self.chatSenderIsVerified
			or self.chatSenderIsAdmin
		):
			text = sender + text
	...
	def readChatBanner(self):
		...
		self.chatBannerSearchObject = search(
			banner,
			lambda obj: bool(obj.name) and obj.role != role("LINK"),
			lambda banner: ui.message(banner.name if banner.role != role("BUTTON") else _("Chat banner")),
			continueOnFound=True,
		)
